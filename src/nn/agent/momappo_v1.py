...
from momaland.learning.cooperative_momappo.discrete_momappo import train
import argparse
import time
...
from src.all_modules import all_environments
...
def parse_args():
    ...
    parser.add_argument("--save-policies", type=lambda x: bool(strtobool(x)), default=False, nargs="?", const=True,
                        help="save the trained policies")
    ...
    parser.add_argument("--weights-generation", type=str, default="OLS",
                        help="The method to generate the weights - 'OLS' or 'uniform'")
    parser.add_argument("--num-steps-per-epoch", type=int, default=128,
                        help="the number of steps per epoch (higher batch size should be better)")
...
class GRU(nn.Module):
    ...
    @nn.compact
    def __call__(self, x):
        gru = nn.GRUCell('Ticket_GRU')
        initial_state = self.param('initial_state', initializers.zeros, (self.hidden_size,))
        state = initial_state
...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        ...
        for units in self.net_arch:
            x = nn.Dense(units, kernel_init=orthogonal(jnp.sqrt(2)), bias_init=constant(0.0))(x)
            x = activation(x)
...
class GRUActor(nn.Module):
    """A GRU-based actor for the MOMAPPO algorithm."""

    action_dim: int
    net_arch: Sequence[int]
    activation: str = "relu"

    current_tech_features_size: int = 1
    previous_ticket_features_size: int = 1
    current_ticket_features_size: int = 1
    other_tech_features_size: int = 1
    gru_hidden_size: int = 16

    def setup(self):
        self.gru = GRU(hidden_size=self.gru_hidden_size)
        self.mlp = MLP(net_arch=self.net_arch, action_dim=self.action_dim, activation=self.activation)

    @nn.compact
    def __call__(self, local_obs_and_id: jnp.ndarray):
        """Actor network initialization."""
        # Unpack the observation
        start_prev_ticket = self.current_tech_features_size
        end_prev_ticket = start_prev_ticket + self.previous_ticket_features_size
        start_curr_ticket = end_prev_ticket
        end_curr_ticket = start_curr_ticket + self.current_ticket_features_size
        start_other_tech = end_curr_ticket
        end_other_tech = start_other_tech + self.other_tech_features_size

        previous_ticket_features = local_obs_and_id[start_prev_ticket:end_prev_ticket]
        current_ticket_features = local_obs_and_id[start_curr_ticket:end_curr_ticket]
        current_tech_features = local_obs_and_id[:self.current_tech_features_size]
        other_technicians_features = local_obs_and_id[start_other_tech:end_other_tech]
        id_feature = local_obs_and_id[-1:]

        # Pass the previous_ticket_features through the GRU
        gru_output = self.gru(previous_ticket_features)

        # Concatenate the GRU output with the other features
        x = jnp.concatenate([current_tech_features, gru_output, current_ticket_features,
                             other_technicians_features, id_feature], axis=-1)
...
class GRUCritic(nn.Module):
    """A GRU-based critic for the MOMAPPO algorithm."""
    ...
    current_tech_features_size: int = 1
    previous_ticket_features_size: int = 1
    other_features_size: int = 1
    number_of_agents: int = 1

    gru_hidden_size: int = 16

    def setup(self):
        self.tech_gru = [GRU(hidden_size=self.gru_hidden_size) for _ in range(self.number_of_agents)]
        self.mlp = MLPCritic(net_arch=self.net_arch, output_dim=1, activation=self.activation)
    ...
    def __call__(self, global_obs: jnp.ndarray):
        ...
        tech_features = [global_obs[i:i + self.current_tech_features_size] for i in
                         range(0, len(global_obs) - self.other_features_size, step_size)]
        ticket_features = [global_obs[i + self.current_tech_features_size:i + step_size] for i in
                           range(0, len(global_obs) - self.other_features_size, step_size)]

        gru_outputs = [gru(ticket_features[i]) for i, gru in enumerate(self.tech_gru)]
        x = jnp.concatenate([jnp.concatenate([tech_features[i], gru_outputs[i]]) for i in range(len(tech_features))])
        x = jnp.concatenate([x, global_obs[-self.other_features_size:]])
...
def _update_minbatch(actor_critic_train_state, batch_info):
    ...
    total_loss_and_debug, grads = grad_fn(actor_train_state.params, critic_train_state.params, traj_batch, advantages,
                                          targets)
...
if __name__ == "__main__":
    ...
    # Env init
    env: ParallelEnv = env_constructor(**env_args)
    eval_env: ParallelEnv = env_constructor(**env_args)
    ...
    actor_features_sizes = env.unwrapped.actor_features_sizes
    critic_features_sizes = env.unwrapped.critic_features_sizes
    single_action_space = env.action_space(env.possible_agents[0])
    actor = GRUActor(single_action_space.n, net_arch=args.actor_net_arch, activation=args.activation,
                     **actor_features_sizes)
    critic = GRUCritic(net_arch=args.critic_net_arch, activation=args.activation, **critic_features_sizes)
